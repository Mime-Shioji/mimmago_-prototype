 ...
 * Displays individual chat messages with department information
 * チャットメッセージと窓口情報を表示するコンポーネント
 */
...
export function ChatMessage({ role, content, department, phone, hours, location }: ChatMessageProps) {
  ...
  return (
    <div className={`flex gap-2.5 ${role === "user" ? "justify-end" : "justify-start"}`}>
      ...
      <div className={`flex flex-col gap-3 max-w-[85%] ${role === "user" ? "items-end" : "items-start"}`}>
        <div
